...
describe("the thousand-plant sector", () => {
  ...
  it("only says you scanned it all when nothing is outstanding", () => {
    expect(tierFor(facts({ visited: true, scannedByYou: 1000, unscannedByYou: 0, knownBodies: 1000 }))).toBe(
      "done",
    );
  });
  ...
  it("does not call it done when the corpus knows of more than you scanned", () => {
    ...
    expect(tierFor(facts({ visited: true, scannedByYou: 1, unscannedByYou: 0, knownBodies: 378 }))).toBe(
      "confirmed",
    );
  });
